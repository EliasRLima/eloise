...
<div class="widget-box">
	...
	<div class="widget-body">
		<div class="widget-main" style="height: 650px;">

						<form name="formQuantitativos" method="POST" action="relatorio_quantidade.php">
		    									<?php
			 											$link = @mysqli_connect('LocalHost','root','','eloisebd') or die("sem conexao");
													    $queryTOTAL = "select (select count(id) from pessoa where id not in (select estagiario_id from estagio ) and tipo_papel=3)as candidatos, (select count(id) from pessoa where id in (select estagiario_id from estagio) and tipo_papel=3) as estagiarios,(select count(id) from estagio) as estagios from pessoa limit 1;";
														$total = @mysqli_query($link,$queryTOTAL);
														$totalr = @mysqli_fetch_assoc($total);
												?>
												<h3 ><p>Numeros totais:</p></h3>
												<p>
													<table style="width:100%" >
														...
														<tr>
															<td><?=$totalr['candidatos']?></td>
															<td><?=$totalr['estagiarios']?></td>
															<td><?=$totalr['estagios']?></td>
														</tr>
													</table>
												</p>
												
												<h3 >Utilize os campos para tentar gerar um quantitativo especifico:</h3>
												<?
		    									...
		    									?>
  												<Label><b>Empresa(s):</b></label>
  												<br><input style="width:100%" type="text" name="emp_string" id="emp_string" placeholder="Filtrar pelo nome"/></br>
  												<Label><b>Servidor(es):</b><br></label>
  												<input style="width:100%" type="text" name="serv_string" id="serv_string" placeholder="Filtrar pelo nome"/>
												</br>
												<br>
												<input style="width:75%;" class="btn btn-success btn-lg" type="submit" name="buscar" id="button_buscar" value="Filtrar"/>
												</br>
							</form>

...
<div class="col-xs-12">
 ...
 <?php 
	...
	$lp = $cpessoa->readPessoa($c, $crud, $colunas="id, nome as descricao, cadastro, tipo_papel, tipo_cadastro", $tabela="pessoa", $tipoMetodo=2, $filtro="tipo_cadastro IN (1,2) and id=$pessoa_id", $viewsql=0);

	$lgp = $cpessoa->readPessoa($c, $crud, $colunas="id, pessoa_id, nivel", $tabela="login", $tipoMetodo=2, $filtro="pessoa_id=$pessoa_id", $viewsql=0);

	$lat = $cpessoa->readPessoa($c, $crud, $colunas="pessoa_id, ativo", $tabela="login", $tipoMetodo=2, $filtro="pessoa_id=$pessoa_id", $viewsql=0);

	$lmat = $cpessoa->readPessoa($c, $crud, $colunas="pessoa_id, autenticador as descricao", $tabela="login", $tipoMetodo=2, $filtro="pessoa_id=$pessoa_id", $viewsql=0);

	$lse = $cpessoa->readPessoa($c, $crud, $colunas="pessoa_id, senha", $tabela="login", $tipoMetodo=2, $filtro="pessoa_id=$pessoa_id", $viewsql=0);
...
<script>
	...
	$(document).ready(function(){
		...
		$(window).load(function(){
			...
			$("#divpessoaid").append('<input type="hidden" id="_pessoa_id" name="_pessoa_id" value="<?php echo $pessoa_id; ?>"/>');

	...
	<div class="widget-body">
		<div class="widget-main">
			...
			<?
				...
				$query = "select * from pessoa_inativos where id = '".$id."'";
				$resultado = @mysqli_query($link,$query);
				$num_rows = @mysqli_num_rows($resultado);
				$linha = @mysqli_fetch_assoc($resultado);
				if($num_rows==1){
					?>
					<br><h4>Aten&ccedil;&atilde;o! <?=$linha['nome']?> de codigo " <?=$linha['cadastro']?> " ser&aacute; movido(a) para a parte ativa do sistema!</h4></br>
					<?php $url="_id=".$linha['id'];?>
					<form name="formNomeBusca" method="POST" action="reativar.php?url=<?php echo $inf->doCodifica($url)?>" style="">
					<br><label>Qual motivo para reativa-lo(a)?</label></br>
					<input style="width:80%;margin:3px;" type="text" name="obs" id="obs" placeholder="Digite o motivo"></input><input style="width:15%;" class="btn btn-success btn-lg" type="submit" name="button" id="button" value="Confirmar"/>
				    </form>
					<?
				}
